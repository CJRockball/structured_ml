...
version = 'v4'
pipe_log.info(f"fe_{version} feature engineering stage started")
set_seed()
...
RATIO_PAIRS = [{'u','redshift'},('g','redshift'),('r','redshift'),('i','redshift'),('z','redshift')]  # e.g. [("triglycerides", "hdl_cholesterol"), ("bmi", "age")]
...
# set up interactions
NUM_NUM_PAIRS = [('g_minus_r', 'r_minus_i'), ('u_minus_g','g_minus_r'), ('g_minus_r','r'), ('u_minus_g','g'),
                 {'u','redshift'},('g','redshift'),('r','redshift'),('i','redshift'),('z','redshift'),]  # e.g. [("bmi", "age"), ("triglycerides", "bmi")]
...
save_dataset(
    df=df_train,
    path=PROC_DIR / f"{comp_cfg.name}_train_{sname}_{version}_{DATE_SLUG}.parquet",
...
save_dataset(
    df=df_test,
    path=PROC_DIR / f"{comp_cfg.name}_test_{sname}_{version}_{DATE_SLUG}.parquet",
...
print_lineage(PROC_DIR / f"{comp_cfg.name}_train_{sname}_{version}_{DATE_SLUG}.parquet")

pipe_log.info(f"fe_{version}_ stage complete — "
